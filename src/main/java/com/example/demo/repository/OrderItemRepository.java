...
public interface OrderItemRepository extends JpaRepository<OrderItem, Long> {
    ...
    @Query("SELECT oi FROM OrderItem oi WHERE oi.donHang.id = :orderId")
    List<OrderItem> findByDonHangId(@Param("orderId") Long orderId);
    
    @Query("SELECT SUM(oi.soLuong * oi.giaBan) FROM OrderItem oi WHERE oi.donHang.id = :orderId")
