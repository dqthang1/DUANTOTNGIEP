...
public class WishlistController {
    ...
    public ResponseEntity<Map<String, Object>> toggleWishlist(
        ...
        try {
            ...
            log.info("Toggle wishlist - User: {}, Product: {}, Current status: {}", user.getId(), productId, isInWishlist);
            
            boolean result;
            if (isInWishlist) {
                result = yeuThichService.removeFromWishlist(user.getId(), productId);
                log.info("Removing from wishlist - Result: {}", result);
            } else {
                result = yeuThichService.addToWishlist(user.getId(), productId);
                log.info("Adding to wishlist - Result: {}", result);
            }
            
            if (result) {
                // Kiểm tra lại trạng thái sau khi thực hiện
                boolean newStatus = yeuThichService.isInWishlist(user.getId(), productId);
                
                Map<String, Object> response = new HashMap<>();
                response.put("success", true);
                response.put("isInWishlist", newStatus);
                response.put("message", newStatus ? "Đã thêm vào yêu thích" : "Đã xóa khỏi yêu thích");
                
                log.info("Wishlist toggle response - Old status: {}, New status: {}, message: {}", isInWishlist, newStatus, response.get("message"));
                return ResponseEntity.ok(response);
            } else {
