...
public class WebController {
    ...
    public String productDetails(@PathVariable Long id, Model model, Authentication authentication) {
        try {
            ...
            // Load product details WITH all relationships (danhMuc, thuongHieu, monTheThao, anhSanPhams, bienTheSanPhams)
            var productOpt = productService.findByIdWithDetails(id);
            if (productOpt.isPresent()) {
                ...
                // Debug logging
                log.info("Product loaded: ID={}, Name={}, BienTheSanPhams size={}", 
                    product.getId(), product.getTen(), 
                    product.getBienTheSanPhams() != null ? product.getBienTheSanPhams().size() : "null");
            } else {
            ...
            log.error("Error loading product details for ID: {}", id, e);
            return "redirect:/products";
        }
    }
