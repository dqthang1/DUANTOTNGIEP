...
public class WebController {
    ...
    public String home(Model model, Authentication authentication) {
        ...
        // Add data for mega menu and home page
        try {
            ...
            // Continue without the data - page will still work
        }
    ...
    public ResponseEntity<String> testProduct(@PathVariable Long id) {
        try {
            log.info("Testing product with ID: {}", id);
            var productOpt = productService.findById(id);
            if (productOpt.isPresent()) {
                Product product = productOpt.get();
                return ResponseEntity.ok("Product found: " + product.getTen());
            } else {
                return ResponseEntity.ok("Product not found with ID: " + id);
            }
        } catch (Exception e) {
            log.error("Error testing product", e);
    ...
    public String registerUser(
        ...
        try {
            ...
            if (userService.findByEmail(email).isPresent()) {
                model.addAttribute("error", "Email này đã được sử dụng! Vui lòng chọn email khác hoặc đăng nhập nếu đã có tài khoản.");
                model.addAttribute("email", email); // Giữ lại email để user có thể sửa
                return "register";
            }
            ...
            // Set default role với error handling
            try {
                newUser.setVaiTro(userService.getDefaultRole());
            } catch (Exception e) {
                log.error("Error setting default role for user: {}", email, e);
    ...
    private User getCurrentUser(Authentication authentication) {
        ...
        try {
            ...
            if (user.getVaiTro() == null) {
                log.warn("User {} has no role, setting default role", email);
                try {
                    user.setVaiTro(userService.getDefaultRole());
                    userService.save(user);
                } catch (Exception e) {
                    log.error("Error setting default role for user: {}", email, e);
                }
    ...
    public String productDetails(@PathVariable Long id, Model model, Authentication authentication) {
        try {
            ...
            List<Map<String, Object>> relatedProducts = productService.getRelatedProducts(id, 6);
            
            // Get mock reviews
            List<Map<String, Object>> reviews = productService.getMockReviews();
            ...
            log.info("Product detail page loaded for product ID: {}", id);
    ...
    public ResponseEntity<Map<String, Object>> checkEmail(@RequestParam String email) {
        try {
            ...
            response.put("message", exists ? "Email này đã được sử dụng" : "Email có thể sử dụng");
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            log.error("Error checking email: {}", email, e);
            Map<String, Object> response = new HashMap<>();
            response.put("exists", false);
            response.put("message", "Lỗi kiểm tra email");
