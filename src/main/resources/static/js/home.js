...
class HomePage {
    ...
    createProductCard(product) {
        ...
        // Badge logic: prioritize NEW ARRIVAL for consistency with reference
        const isNewProduct = product.ngayTao && new Date(product.ngayTao) > new Date(Date.now() - 30 * 24 * 60 * 60 * 1000); // 30 days
        const badgeHtml = isNewProduct 
            ? `<div class="plp-badge new-arrival">NEW ARRIVAL</div>`
            : discountPercent > 0 
            ? `<div class="plp-badge discount">-${discountPercent}%</div>`
            : '';
        ...
        return `
            <a href="/products/${product.id}" class="${cardClass}" data-product-id="${product.id}">
                <div class="plp-product-image">
                    ...
                    <!-- Quick Actions - Single pill container with 2 icon-only buttons -->
                    <div class="quick-actions">
                        <button class="quick-action-btn btn-options" data-product-id="${product.id}" title="Tùy chọn" aria-label="Tùy chọn – ${cleanName}">
                            <svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6V4m0 2a2 2 0 100 4m0-4a2 2 0 110 4m-6 8a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4m6 6v10m6-2a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4"></path>
                            </svg>
                        </button>
                        <button class="quick-action-btn btn-quick-view" data-product-id="${product.id}" title="Xem nhanh" aria-label="Xem nhanh – ${cleanName}">
                            <svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        try {
            ...
            if (data.success) {
                ...
                if (data.message && data.message.includes('đăng nhập')) {
                    this.showError('Vui lòng <a href="/login" style="color: inherit; text-decoration: underline;">đăng nhập</a> để thêm sản phẩm vào giỏ hàng');
                } else {
    ...
    async handleNewsletterSubmit(e) {
        ...
        try {
            ...
            if (data.success) {
                this.showSuccess('Gửi thành công! Kiểm tra email.');
                e.target.reset();
            } else {
...
function getProductImage(product) {
    // Ưu tiên ảnh từ anhSanPhams array
    if (product.anhSanPhams && product.anhSanPhams.length > 0) {
        return product.anhSanPhams[0].duongDan || product.anhSanPhams[0].duongDanAnh;
    }
    // Fallback về anhChinh
    if (product.anhChinh) {
        return product.anhChinh;
    }
    // Fallback cuối cùng
...
// Helper function to get product thumbnails
function getProductThumbnails(product) {
    if (product.anhSanPhams && product.anhSanPhams.length > 0) {
        return product.anhSanPhams.slice(0, 4).map((img, index) => `
            <div class="thumbnail ${index === 0 ? 'active' : ''}" onclick="changeMainImage('${img.duongDan || img.duongDanAnh}')">
                <img src="${img.duongDan || img.duongDanAnh}" alt="Thumbnail ${index + 1}">
            </div>
        `).join('');
    }
    
    // Nếu có anhChinh, tạo thumbnail từ ảnh chính
    if (product.anhChinh) {
        return `
            <div class="thumbnail active" onclick="changeMainImage('${product.anhChinh}')">
                <img src="${product.anhChinh}" alt="Main image">
            </div>
        `;
    }
    
    // Fallback cuối cùng
    return '<div class="thumbnail active"><img src="/images/no-image.svg" alt="No image"></div>';
}
...
window.changeMainImage = function(imageSrc) {
    ...
    const clickedThumbnail = Array.from(thumbnails).find(thumb => 
...
// Global function to close quick view modal
...
// Global function to add to cart from quick view
window.addToCartFromQuickView = function(productId) {
    ...
    const selectedColor = document.querySelector('.color-btn.active')?.dataset.color || 'Đen';
    const quantity = parseInt(document.querySelector('.qty-display')?.textContent || '1');
...
// Global function to change quantity
...
window.quickViewProduct = function(productId) {
    ...
    Promise.all([
        ...
        fetch(`/api/products/${productId}/variants`).then(response => response.json())
    ])
    .then(([productData, variantData]) => {
        if (productData.success && variantData.success) {
            showQuickViewModal(productData.product, variantData);
        } else {
    ...
    .catch(error => {
        ...
        showToast('Không thể tải thông tin sản phẩm. Vui lòng thử lại sau.', 'error');
    });
};

function showQuickViewModal(product, variantData) {
    // Tạo modal quick view với dữ liệu thật
    const modal = document.createElement('div');
    modal.className = 'quick-view-modal';
    
    // Lấy dữ liệu biến thể thật
    const availableSizes = variantData.sizes || ['S', 'M', 'L', 'XL'];
    const availableColors = variantData.colors || [product.mauSac || 'Đen'];
            ...
            <div class="modal-body">
                <div class="product-images">
                    <div class="main-image">
                        ...
                        ${product.giamGia ? `<div class="product-badge sale">-${product.giamGia}%</div>` : ''}
                        ${product.moi ? '<div class="product-badge new">MỚI</div>' : ''}
                    </div>
                    <div class="thumbnail-images">
                        ${getProductThumbnails(product)}
                    </div>
                </div>
                <div class="product-details">
                    ...
                    <div class="product-price">
                        ${product.giaGoc && product.giaGoc > product.gia ? 
                            `<span class="price-original">${formatPrice(product.giaGoc)}</span>` : ''
                        }
                        <span class="price-current">${formatPrice(product.gia)}</span>
                        ${product.giamGia ? `<span class="price-discount">-${product.giamGia}%</span>` : ''}
                    </div>
                        ...
                        <div class="option-group">
                            ...
                            <div class="color-options">
                                ${availableColors.map((color, index) => `
                                    <button class="color-btn ${index === 0 ? 'active' : ''}" data-color="${color}" 
                                            style="background: ${getColorHex(color)};" title="${color}"></button>
                                `).join('')}
                            </div>
                        </div>
...
window.toggleWishlist = function(productId) {
    ...
    .catch(error => {
        ...
        showToast('Không thể thêm vào yêu thích. Vui lòng thử lại sau.', 'error');
    })
    .finally(() => {
