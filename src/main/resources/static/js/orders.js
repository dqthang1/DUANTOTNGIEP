...
class OrdersManager {
    constructor() {
        ...
        this.allOrders = []; // Initialize to empty array
        this.init();
    }
    ...
    filterOrders(status) {
        ...
        if (status !== 'all') {
            filteredOrders = this.allOrders.filter(order => {
                // Handle both Vietnamese and English status codes
                switch (status) {
                    case 'PENDING':
                        return order.trangThai === 'PENDING' || order.trangThai === 'CHO_XAC_NHAN';
                    case 'CONFIRMED':
                        return order.trangThai === 'CONFIRMED' || order.trangThai === 'DANG_CHUAN_BI';
                    case 'DANG_GIAO':
                        return order.trangThai === 'DANG_GIAO' || order.trangThai === 'SHIPPING';
                    case 'DA_GIAO':
                        return order.trangThai === 'DA_GIAO' || order.trangThai === 'DELIVERED';
                    case 'DA_HUY':
                        return order.trangThai === 'DA_HUY' || order.trangThai === 'CANCELLED';
                    default:
                ...
                <div class="order-header">
                    <div class="row align-items-center">
                        <div class="col-md-6">
                            ...
                            <small class="text-muted">${this.formatDate(order.ngayDatHang)}</small>
                        </div>
                                ...
                                ${order.orderItems.slice(0, 3).map(item => `
                                    <div class="order-item">
                                        <div class="d-flex align-items-center">
                                            <img src="${item.productImage || (item.sanPham && item.sanPham.anhChinh) || '/images/no-image.svg'}" 
                                                 alt="${item.productName || (item.sanPham && item.sanPham.ten) || 'Product'}" 
                                                 class="product-image me-3">
                                            <div class="flex-grow-1">
                                                <h6 class="mb-1">${item.productName || (item.sanPham && item.sanPham.ten) || 'Product'}</h6>
                                                ${item.kichCo || item.kichThuoc ? `<small class="text-muted">Kích cỡ: ${item.kichCo || item.kichThuoc}</small><br>` : ''}
                                                ${item.mauSac ? `<small class="text-muted">Màu sắc: ${item.mauSac}</small><br>` : ''}
                                                <small class="text-muted">Số lượng: ${item.soLuong}</small>
                                            </div>
                                            <div class="text-end">
                                                <div class="fw-bold">${this.formatCurrency(item.thanhTien || (item.donGia || item.giaBan) * item.soLuong)}</div>
                                            </div>
                                        </div>
                                    </div>
                                `).join('')}
                ...
                <div class="col-md-6">
                    ...
                    <table class="table table-sm">
                        ...
                        <tr>
                            ...
                            <td>${this.formatDate(order.ngayDatHang)}</td>
                        </tr>
                        <tr>
                            ...
                            <tr>
                                <td>
                                    <div class="d-flex align-items-center">
                                        ...
                                        <span>${item.productName || (item.sanPham && item.sanPham.ten) || 'Product'}</span>
                                    </div>
                                </td>
                                <td>${item.kichCo || item.kichThuoc || '-'}</td>
                                <td>${item.mauSac || '-'}</td>
                                <td>${item.soLuong}</td>
                                <td>${this.formatCurrency(item.donGia || item.giaBan)}</td>
                                <td>${this.formatCurrency(item.thanhTien || (item.donGia || item.giaBan) * item.soLuong)}</td>
                            </tr>
                        `).join('')}
                    </tbody>
                </table>
