...
class CheckoutManager {
    ...
    renderOrderItems(cartItems) {
        console.log('Checkout rendering order items:', cartItems);
    ...
    renderAddresses(addresses) {
        ...
        addressList.innerHTML = addresses.map(address => `
            <div class="address-card ${address.laDiaChiMacDinh ? 'selected' : ''}" 
                ...
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        <h6 class="mb-1">${address.tenNguoiNhan}</h6>
                        <p class="mb-1">${address.soDienThoai}</p>
                        <p class="mb-0 text-muted">${address.diaChiChiTiet}, ${address.quanHuyen}, ${address.tinhThanh}</p>
                    </div>
                    ${address.laDiaChiMacDinh ? '<span class="badge bg-primary">Mặc định</span>' : ''}
                </div>
            </div>
        `).join('');
        ...
        if (addresses.length > 0) {
            const defaultAddress = addresses.find(addr => addr.laDiaChiMacDinh) || addresses[0];
    ...
    formatCurrency(amount) {
        console.log('Checkout formatting currency for amount:', amount, 'type:', typeof amount);
        ...
        if (isNaN(numAmount)) {
            console.warn('Cannot convert to number:', amount);
            return '0 ₫';
        }
